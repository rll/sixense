		...
		for( base=0; base<sixenseGetMaxBases(); base++ ) {
			...
			for( cont=0; cont<sixenseGetMaxControllers(); cont++ ) {
				...
				if( sixenseIsControllerEnabled( cont ) ) {
					...
					fprintf( log_file, "tb_deg: [ %f, %f, %f ]", tb.yaw_deg, tb.pitch_deg, tb.roll_deg);
		...
		for( cont=0; cont<sixenseGetMaxControllers(); cont++ ) {
			...
			if( sixenseIsControllerEnabled( cont ) ) {
				...
				if( cont == 0 ) {
					...
					glColor3d(0.3f*colors[base][0]+flash_multiplier, 0.3f*colors[base][1]+flash_multiplier, 0.3f*colors[base][2]+flash_multiplier );
				}
...
void draw_controller_info() {
	...
	if( graph_mode == 0 )
		shapesPrintf (next_line++, 3, "Press 'g' to cycle through pos, vel and accel graphs" );
	else if( graph_mode == 1 )
		shapesPrintf (next_line++, 3, "Press 'g' to cycle through pos, vel and accel graphs" );
	else if( graph_mode == 2 )
		shapesPrintf (next_line++, 3, "Press 'g' to cycle through pos, vel and accel" );
	else if( graph_mode == 3 )
		shapesPrintf (next_line++, 3, "Press 'g' to cycle through pos, vel and accel" );
		
	if( graph_mode == 0 )
		shapesPrintf (next_line++, 3, "  Graphs are OFF." );
	else if( graph_mode == 1 )
		shapesPrintf (next_line++, 3, "  Currently plotting POSITION." );
	else if( graph_mode == 2 )
		shapesPrintf (next_line++, 3, "  Currently plotting VELOCITY." );
	else if( graph_mode == 3 )
		shapesPrintf (next_line++, 3, "  Currently plotting ACCELERATION." );
		...
		for( base=0; base<sixenseGetMaxBases(); base++ ) {
			...
			for( cont=0; cont<sixenseGetMaxControllers(); cont++ ) {
				...
				if( sixenseIsControllerEnabled( cont ) ) {
					...
					shapesPrintf( next_line + i, 3, "%s (% 6.1f,% 6.1f,% 6.1f) Y: % 6.1f deg P: % 6.1f deg R: % 6.1f deg", ss.str().c_str(),
